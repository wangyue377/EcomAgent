...
class SkillManager:
    ...
    def build_catalog_prompt(self) -> str:
        ...
        lines = [
            "\n\n## 可用技能（Skills）",
            "以下是你可以使用的专业技能，仅在以下场景调用 `load_skill` 加载：",
        ]
        ...
        lines.append("")
        lines.append("### 技能使用规则（重要）")
        lines.append("1. **不要**因为技能存在就加载它——只在用户的问题明确匹配技能描述时才使用")
        lines.append("2. **简单查询不要加载技能**：查订单、查物流、查商品、查政策等单一操作，直接调用对应工具即可，不需要加载技能")
        lines.append("3. **先调用工具，后考虑技能**：先通过 query_order / query_logistics / query_product 等工具获取数据，只有遇到需要多步流程的场景才加载技能")
        lines.append("4. 加载技能后，按指令流程使用已有工具完成操作")
        lines.append("5. 如果不匹配任何技能，照常回答即可，不必强行使用技能")
