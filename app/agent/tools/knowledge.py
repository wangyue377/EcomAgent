...
import os
# 强制离线模式：sentence-transformers 不联网下载
...
def _create_embedder_for_backend():
    """根据后端类型创建合适的 Embedder。

    - chroma 后端：使用 sentence-transformers 本地中文模型，纯离线
    - numpy 后端：走 API Embedder（需服务商支持 embedding 接口）
    """
    if settings.rag_backend.lower() == "chroma":
        ...
        class _LocalEmbedder:
            """包装 sentence-transformers，适配 Embedder 接口。"""
            def __init__(self):
                self._model = model
...
def _get_retriever() -> KnowledgeRetriever:
    ...
    if _retriever is None:
        embedder = _create_embedder_for_backend()
