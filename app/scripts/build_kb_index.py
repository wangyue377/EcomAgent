...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="使用本地 sentence-transformers 模型（如 shibing624/text2vec-base-chinese），无需 API",
    )
    ...
    if args.local_model:
        ...
        print(f"   使用本地模型: {args.local_model}")
        model = SentenceTransformer(args.local_model)
        vectors = model.encode([c.text for c in chunks]).tolist()
    elif args.local_embed:
        # 兼容旧参数 --local-embed
        from chromadb.utils.embedding_functions import DefaultEmbeddingFunction

        ef = DefaultEmbeddingFunction()
        print(f"   使用本地 embedding（{ef.__class__.__name__}）")
        vectors = ef([c.text for c in chunks])
    else:
        embedder = Embedder(
