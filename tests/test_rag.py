...
def _make_embedder():
    """创建 embedder。若 embedding_model 是本地模型名则使用 sentence-transformers。"""
    if settings.embedding_model and not settings.embedding_model.startswith("text-"):
...
# ---------- 测试 2：构建/复用索引 ----------
def _build_index(backend_name: str):
...
# ---------- 测试 3：Top-K 召回 ----------
def _retriever_top_k(backend_name: str):
...
# ---------- 测试 5：Agent 端到端 ----------
def _agent_end_to_end(backend_name: str):
...
def run_for_backend(backend_name: str):
    ...
    print("=" * 60)
    _test_chunker()
    _build_index(backend_name)
    _retriever_top_k(backend_name)
    _tool_shape(backend_name)
    _agent_end_to_end(backend_name)
...
def test_rag_with_chroma():
    """pytest 入口：用 chroma 后端跑 RAG 测试。"""
    run_for_backend("chroma")
