...
def test_multi_turn():
    ...
    # 验证对话历史长度（至少包含 2 user + 2 assistant）
    actual_len = len(agent.raw_messages)
    assert actual_len >= 4, (
        f"对话历史长度异常: {actual_len}, 期望至少 4 条"
    )
    print(f"\n[PASS] 对话历史长度正常: {actual_len} 条消息")
    return True
...
def test_reset():
    ...
    assert len(agent.raw_messages) >= 2  # user + assistant

    agent.reset()
    assert len(agent.raw_messages) == 0  # 全部清空
