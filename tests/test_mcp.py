...
# 本地注册的所有工具（registry.py）
EXPECTED_LOCAL_TOOLS = {
    "query_order", "query_product", "query_logistics",
    "apply_refund", "search_knowledge", "list_user_orders",
    "recall_user_memory", "load_skill",
}
# MCP Server 暴露的工具（server.py）
EXPECTED_MCP_TOOLS = {"query_order", "query_product", "query_logistics", "apply_refund", "search_knowledge"}


def _check_mcp_server() -> bool:
    """检查 MCP Server 是否在运行，避免连接测试硬失败。"""
    import socket
...
def test_tool_manager_mcp():
    ...
    try:
        ...
        if len(defs) not in (5, 8):  # 5=MCP直连, 8=降级到本地
            _fail(f"工具定义数量异常：{len(defs)}")
...
def test_tool_manager_local():
    ...
    try:
        defs = manager.tool_definitions
        if len(defs) != len(EXPECTED_LOCAL_TOOLS):
            _fail(f"期望 {len(EXPECTED_LOCAL_TOOLS)} 个工具定义，实际 {len(defs)}")
